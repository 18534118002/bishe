...
import com.yanxiuhair.common.utils.CacheUtils;
import com.yanxiuhair.common.utils.StringUtils;
import com.yanxiuhair.system.domain.SysConfig;
import com.yanxiuhair.system.mapper.SysConfigMapper;
...
public class SysConfigServiceImpl implements ISysConfigService {
	...
	public String selectSysCodeConfigByKey(String configKey) {
		...
		SysConfig retConfig = configMapper.selectConfig(config);
		if (StringUtils.isNotNull(retConfig)) {
			int num = Integer.parseInt(retConfig.getConfigValue().substring(retConfig.getConfigValue().length()-5, retConfig.getConfigValue().length()));
			if(num + 1 == 99999){
				num = 0;
			}
			String sysCode = retConfig.getConfigValue().substring(0,retConfig.getConfigValue().length()-5) + String.format("%05d", num + 1);
			retConfig.setConfigValue(sysCode);
